...
public class GoogleCalendar implements CalendarTarget {
    ...
    public GoogleCalendar(JSONObject settings) throws Exception {
        String proxyHost = getNonEmptyStringOrNull(settings, "proxyHost");
        if (proxyHost != null) {
            System.setProperty("https.proxyHost", proxyHost);

            String proxyPort = getNonEmptyStringOrNull(settings, "proxyPort");
            if (proxyPort != null) {
                System.setProperty("https.proxyPort", proxyPort);
            }

            String proxyUser = getNonEmptyStringOrNull(settings, "proxyUser");
            if (proxyUser != null) {
                String rawProxyPw = getNonEmptyStringOrNull(settings, "proxyPw");
                String proxyPw = rawProxyPw == null ? "" : Encryption.decrypt(rawProxyPw);
    ...
    private static String getNonEmptyStringOrNull(JSONObject settings, String key) {
        if (!settings.has(key)) {
            return null;
        }
    ...
    public void delete(AbstractEvent toDelete) throws IOException {
        if (!(toDelete instanceof GoogleEvent)) {
